#include "Input.hpp"
#include "Logger.hpp"
#define CAST_KEY(X) static_cast<Z_UINT16>(X)
#define CAST_MOUSE(X) static_cast<Z_UINT8>(X)


namespace ZE {
  std::array<bool, Input::MAX_KEYS> Input::s_CurrentKeys{};
  std::array<bool, Input::MAX_KEYS> Input::s_PreviousKeys{};
  std::array<bool, Input::MAX_BUTTONS> Input::s_CurrentMouse{};
  std::array<bool, Input::MAX_BUTTONS> Input::s_PreviousMouse{};
  ST_VEC2() Input::s_MousePos{};
  ST_VEC2() Input::s_MouseWheel{};
  ...
  void Input::OpenGamepad(const SDL_JoystickID id) {
  ...
  bool Input::IsKeyDown(Enums::ZE_Keys key) {
    return s_CurrentKeys[CAST_KEY(key)];
  }
  bool Input::IsKeyPressed(Enums::ZE_Keys key) {
    const auto int_key = CAST_KEY(key);
    return s_CurrentKeys[int_key] && !s_PreviousKeys[int_key];
  }
  bool Input::IsKeyReleased(Enums::ZE_Keys key) {
    const auto int_key = CAST_KEY(key);
    return !s_CurrentKeys[int_key] && s_PreviousKeys[int_key];
  }

  bool Input::IsMouseDown(Enums::ZE_Mouse button) {
    return s_CurrentMouse[CAST_MOUSE(button)];
  }
  bool Input::IsMousePressed(Enums::ZE_Mouse button) {
    const auto int_button = CAST_MOUSE(button);
    return s_CurrentMouse[int_button] && !s_PreviousMouse[int_button];
  }
  bool Input::IsMouseReleased(Enums::ZE_Mouse button) {
    const auto int_button = CAST_MOUSE(button);
    return !s_CurrentMouse[int_button] && s_PreviousMouse[int_button];
  }

  ST_VEC2() Input::MousePosition() {
    return s_MousePos;
  }
