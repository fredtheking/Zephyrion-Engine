...
void ZE::Logger::PrintTimestamp(CREF(std::string) timestamp) {
...
std::string ZE::Logger::GetTimestamp() {
...
void ZE::Logger::RawPrint(CREF(std::string) prefix, CREF(std::string) msg, CREF(ST::Color) fg_color, CREF(ST::Color) bg_color) {
...
void ZE::Logger::DebugLog(CREF(std::string) msg, CREF(ST::Color) fg_color) {
  RawPrint("debug", msg, fg_color, Colors::Blank);
}
void ZE::Logger::Information(CREF(std::string) msg) {
  RawPrint("info", msg, Colors::White, Colors::Blank);
}
void ZE::Logger::Warning(CREF(std::string) msg) {
  RawPrint("warn", msg, Colors::Yellow, Colors::Blank);
}
void ZE::Logger::Success(CREF(std::string) msg) {
  RawPrint("success", msg, Colors::Lime, Colors::Blank);
}
void ZE::Logger::Error(CREF(std::string) msg) {
  RawPrint("error", msg, Colors::Red, Colors::Blank);
}
// Also crashes the app.
void ZE::Logger::Critical(CREF(std::string) msg) {
  RawPrint("crit", msg, Colors::Black, Colors::Red);
  std::exit(-1);
}


void ZE::Logger::Separator(CREF(std::string) msg, const char sign) {
  Separator(Colors::Purple, msg, sign);
}
void ZE::Logger::Separator(CREF(ST::Color) fg_color, CREF(std::string) msg, const char sign) {
