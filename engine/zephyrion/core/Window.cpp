...
namespace ZE {
  void Window::Internal_UpdateWindowPosition() const {
    ST_VEC2(int) pos;
  ...
  void Window::Internal_UpdateWindowSizeConfigs() const {
  ...
  void Window::Render() {
    if (m_Imgui) DEFINE_IMIO_VARIABLE
  ...
  void Window::UpdateIcon(CREF(STR) filepath) {
    if (filepath.empty()) {
      if (p_Icon)
  ...
  void Window::UpdatePosition(const Enums::ZE_WindowPosition position_mode) const {
    p_Config->position_mode_enum = position_mode;
    p_Config->position_vec2 = NONVALID_ST_VEC2;
    Internal_UpdateWindowPosition();
  }
  void Window::UpdatePosition(CREF(ST_VEC2(int)) position) const {
    p_Config->position_mode_enum = Enums::ZE_WindowPosition::Custom;
    p_Config->position_vec2 = position;
    Internal_UpdateWindowPosition();
  }
  void Window::UpdatePosition(CREF(ST_VEC2()) position) const {
    Logger::Warning("Clipping floats to ints. Be careful!");
    UpdatePosition(ST_VEC2(int){
      static_cast<int>(position.x),
      static_cast<int>(position.y)
    });
  }
  void Window::UpdatePosition(const int x, const int y) const {
    UpdatePosition(ST_VEC2(int){x, y});
  }
  void Window::UpdatePosition(const float x, const float y) const {
    UpdatePosition(ST_VEC2(){x, y});
  }

  void Window::UpdateSize(CREF(ST_VEC2(int)) size) const {
    p_Config->size_vec2 = size;
    Internal_UpdateWindowSizeConfigs();
    Internal_UpdateWindowSize();
  }
  void Window::UpdateSize(CREF(ST_VEC2())size) const {
    Logger::Warning("Clipping floats to ints. Be careful!");
    UpdateSize(ST_VEC2(int){
      static_cast<int>(size.x),
      static_cast<int>(size.y)
    });
  }
  void Window::UpdateSize(const int width, const int height) const {
    UpdateSize(ST_VEC2(int){width, height});
  }
  void Window::UpdateSize(const float width, const float height) const {
    UpdateSize(ST_VEC2(){width, height});
  }
  ...
  void Window::SetBorderless(const bool value) const {
